...
2. [EPICS Base](epics-base.zh-CN.md) —— 其余部分的构建基础。
3. [IOC 应用](ioc.zh-CN.md) —— 平时打交道最多的部分。
4. [IOC 端口分配](port-allocation.zh-CN.md) —— 与 IOC 文档配合阅读。

剩下两篇是参考手册：写 recipe 时看[支持模块](modules.zh-CN.md)，出问题时看
[排障](troubleshooting.zh-CN.md)。

## 约定

* 每篇文档都有英文版（`docs/`）和中文版（`docs/zh-CN/`，形如 `<name>.zh-CN.md`）；标题下
  第一行是切换另一种语言的链接。
